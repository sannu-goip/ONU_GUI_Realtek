...
{
 if (ip.value=="") {
  alert("<% multilang("1733" "LANG_INVALID_IPV4_ADDR_SHOULD_NOT_EMPTY"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( validateKey( ip.value ) == 0 ) {
  alert("<% multilang("1734" "LANG_INVALID_IPV4_ADDR_SHOULD_BE_DECIMAL_NUM"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if( IsLoopBackIP( ip.value)==1 ) {
  alert("<% multilang("1735" "LANG_INVALID_IPV4_ADDR"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,1,0,255) ) {
  alert("<% multilang("1736" "LANG_INVALID_IPV4_ADDR_1ST_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,2,0,255) ) {
  alert("<% multilang("1737" "LANG_INVALID_IPV4_ADDR_2ND_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,3,0,255) ) {
  alert("<% multilang("1738" "LANG_INVALID_IPV4_ADDR_3RD_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,4,1,254) ) {
  alert("<% multilang("1739" "LANG_INVALID_IPV4_ADDR_4TH_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
...
function checkNetmask(netmask, checkEmpty)
{
 ...
 if (checkEmpty == 1 && netmask.value == "") {
  alert("<% multilang("1740" "LANG_INVALID_IPV4_SUBNET_SHOULD_NOT_EMPTY"); %>");
  netmask.value = netmask.defaultValue;
  netmask.focus();
  return false;
 }
 if (validateKey(netmask.value) == 0) {
  alert("<% multilang("1741" "LANG_INVALID_IPV4_SUBNET_SHOULD_BE_DECIMAL_NUM"); %>");
  netmask.value = netmask.defaultValue;
  netmask.focus();
  return false;
 }
 for (i=1; i<=4; i++) {
  ...
  if( !(d==0 || d==128 || d==192 || d==224 || d==240 || d==248 || d==252 || d==254 || d==255 )) {
   alert("<% multilang("1742" "LANG_INVALID_IPV4_SUBNET_DIGIT"); %>");
   netmask.focus();
   return false;
  }
 }
...
function checkMac(macAddr, checkEmpty)
{
 ...
 if (checkEmpty == 1 && macAddr.value.length == 0) {
  alert("<% multilang("1743" "LANG_INVALID_MAC_ADDR_SHOULD_NOT_EMPTY"); %>");
  return false;
 }
 if (macAddr.value.length > 0 && macAddr.value.length < 12) {
  alert("<% multilang("1744" "LANG_INVALID_MAC_ADDR_NOT_COMPLETE"); %>");
  macAddr.focus();
  return false;
 }
 ...
 if (macdigit == macAddr.value.length || macAddr.value == "000000000000") {
  alert("<% multilang("1745" "LANG_INVALID_MAC_ADDR"); %>");
  macAddr.focus();
  return false;
 }
 for (i=0; i<macAddr.value.length; i++) {
  if ((macAddr.value.charAt(i) >= '0' && macAddr.value.charAt(i) <= '9') ||
   ...
   (macAddr.value.charAt(i) >= 'A' && macAddr.value.charAt(i) <= 'F') )
   continue;
  alert("<% multilang("1746" "LANG_INVALID_MAC_ADDR_SHOULD_BE"); %>");
  macAddr.focus();
  return false;
 }
 return true;
...
{
 if ( !confirm('<% multilang("1749" "LANG_CONFIRM_DELETE"); %>') ) {
  return false;
 }
...
function alertInvalid(fieldname, fieldvalue, additional)
{
 if (additional == undefined)
  alert (fieldname + " " + fieldvalue + "<% multilang("2014" "LANG_INVALID"); %>");
 else
  alert (fieldname + " " + fieldvalue + "<% multilang("2014" "LANG_INVALID"); %>" + additional + ".");
}
...
function validateKeyV6Prefix(str)
{
  ...
  if ( find_flag != 1 ) {
 alert("<% multilang("1750" "LANG_INVALID_IPV6_PREFIX"); %>");
   return 0;
  }
  return 1;
}
...
function timeZoneSelectedValue(){
  // Get the select element by its name attribute
var timeZoneSelect = document.getElementById("timezone_select")[0];
