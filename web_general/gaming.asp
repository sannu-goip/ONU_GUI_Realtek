...
<title><% multilang("25" "LANG_PORT_TRIGGERING"); %><% multilang("238" "LANG_CONFIGURATION"); %></title></head>
<script type="text/javascript" src="share.js"></script>
<script language=javascript>
 function addClick()
 {
   if (formname.ip.value=="") {
  alert('<% multilang("2249" "LANG_IP_ADDRESS_CANNOT_BE_EMPTY"); %>');
  formname.ip.focus();
  return false;
   }
  ...
  alert('<% multilang("2250" "LANG_INVALID_IP_ADDRESS_RANGE_IN_1ST_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
   if ( !checkDigitRange(formname.ip.value,2,0,255) ) {
  alert('<% multilang("2251" "LANG_INVALID_IP_ADDRESS_RANGE_IN_2ND_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
   if ( !checkDigitRange(formname.ip.value,3,0,255) ) {
        alert('<% multilang("2252" "LANG_INVALID_IP_ADDRESS_RANGE_IN_3RD_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
   if ( !checkDigitRange(formname.ip.value,4,1,254) ) {
        alert('<% multilang("2253" "LANG_INVALID_IP_ADDRESS_RANGE_IN_4TH_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
    return true;
 ...
 function formLoad()
 {
  formname.gamelist.options[0] = new Option ("<% multilang("639" "LANG_SELECT_GAME"); %>", -1);
