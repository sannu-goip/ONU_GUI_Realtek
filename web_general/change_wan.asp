...
<style>
  ...
  input:not([type="radio"]):not([type="checkbox"]):not(.link_bg) {
...
<script>
// Function to find all Apply Changes buttons and add scroll-to-top behavior
function addScrollToTopBehavior() {
    ...
    for (var i = 0; i < inputs.length; i++) {
        ...
        // Check if this is an "Apply Changes" button (either format)
        if (input.type === 'submit' && 
            (input.value.includes('LANG_APPLY_CHANGES') || 
             input.value === 'Apply Changes' ||
             input.value.toLowerCase().includes('apply changes'))) {
            ...
            input.addEventListener('click', function(e) {
                ...
                // Small delay to ensure smooth scrolling happens
                setTimeout(function() {
...
<div style="padding-top:2px;"><p class="intro_title">WAN Mode</p></div></div>
...
<p class="intro_title"><% getWanIfDisplay(); %> <% multilang("11" "LANG_WAN"); %></p></div>
 ...
 <table style="margin-top: 5px;">
  <tr>
    <th></th>
   <th colspan=2><select name="lkname" onChange="on_ctrlupdate()" size="1" style="width: 80px;"></th>
  </tr>
  <tr>
