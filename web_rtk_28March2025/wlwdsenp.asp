<%SendWebHeadStr(); %>
<title><% multilang("824" "LANG_WDS_SECURITY_SETTINGS"); %></title>
<SCRIPT>
...
function validateKey_wep(form, idx, str, len, wlan_id)
{
 ...
 alert("<% multilang("2529" "LANG_THE_ENCRYPTION_KEY_YOU_SELECTED_AS_THE__TX_DEFAULT_KEY_CANNOT_BE_BLANK"); %>");
 return 0;
  }
  if (str.length ==0)
   return 1;
  if ( str.length != len) {
   idx++;
 alert("<% multilang("2530" "LANG_INVALID_LENGTH_OF_KEY_CHARACTERS"); %>");
 return 0;
  }
  }
  else {
 if ( str.length != len) {
  alert("<% multilang("2531" "LANG_INVALID_LENGTH_OF_WEP_KEY_VALUE"); %>");
  return 0;
   }
  }
  if ( str == "*****" ||
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("824" "LANG_WDS_SECURITY_SETTINGS"); %> <% if (getIndex("wlan_num")>1) write("-wlan"+(checkWrite("wlan_idx")+1));%></p>
 <p class="intro_content"> <% multilang("825" "LANG_THIS_PAGE_ALLOWS_YOU_SETUP_THE_WLAN_SECURITY_FOR_WDS_WHEN_ENABLED_YOU_MUST_MAKE_SURE_EACH_WDS_DEVICE_HAS_ADOPTED_THE_SAME_ENCRYPTION_ALGORITHM_AND_KEY"); %></p>
</div>
...
<div class="btn_ctl">
 ...
 <input class="link_bg" type="submit" value="<% multilang("150" "LANG_APPLY_CHANGES"); %>" onClick="return saveChanges(document.formWdsEncrypt)">&nbsp;&nbsp;
 <input class="link_bg" type="button" value="<% multilang("743" "LANG_CLOSE"); %>" name="close" OnClick=window.close()>&nbsp;&nbsp;
