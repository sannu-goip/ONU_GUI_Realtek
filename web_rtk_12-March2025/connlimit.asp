...
function addClick()
{
 ...
 alert('<% multilang("2099" "LANG_IP_ADDRESS_CANNOT_BE_EMPTY_IT_SHOULD_BE_FILLED_WITH_4_DIGIT_NUMBERS_AS_XXX_XXX_XXX_XXX"); %>');
 ...
 return false;
  ...
  alert('<% multilang("2100" "LANG_MAX_LIMITATION_PORTS_SHOULD_BE_LOWER_THAN_GLOBAL_TCP_CONNECTION_LIMIT"); %>');
     document.formConnLimitAdd.connnum.focus();
     return false;
    }
    else if ((num4 != 0)&&( num3 == 1)&&( num2 > num4)){
  alert('<% multilang("2101" "LANG_MAX_LIMITATION_PORTS_SHOULD_BE_LOWER_THAN_GLOBAL_UDP_CONNECTION_LIMIT"); %>');
     document.formConnLimitAdd.connnum.focus();
     return false;
    }
  if ( !checkDigitRange(document.formConnLimitAdd.ip.value,1,0,255) ) {
 alert('<% multilang("2102" "LANG_INVALID_IP_ADDRESS_RANGE_IN_1ST_DIGIT_IT_SHOULD_BE_0_255"); %>');
 document.formConnLimitAdd.ip.focus();
 return false;
  }
  if ( !checkDigitRange(document.formConnLimitAdd.ip.value,2,0,255) ) {
 alert('<% multilang("2103" "LANG_INVALID_IP_ADDRESS_RANGE_IN_2ND_DIGIT_IT_SHOULD_BE_0_255"); %>');
 document.formConnLimitAdd.ip.focus();
 return false;
  }
  if ( !checkDigitRange(document.formConnLimitAdd.ip.value,3,0,255) ) {
 alert('<% multilang("2104" "LANG_INVALID_IP_ADDRESS_RANGE_IN_3RD_DIGIT_IT_SHOULD_BE_0_255"); %>');
 document.formConnLimitAdd.ip.focus();
 return false;
  }
  if ( !checkDigitRange(document.formConnLimitAdd.ip.value,4,1,254) ) {
 alert('<% multilang("2105" "LANG_INVALID_IP_ADDRESS_RANGE_IN_4TH_DIGIT_IT_SHOULD_BE_1_254"); %>');
 document.formConnLimitAdd.ip.focus();
 return false;
  }
...
<div class="intro_main ">
 ...
 <p class="intro_content"> <% multilang("631" "LANG_ENTRIES_IN_THIS_TABLE_ALLOW_YOU_TO_LIMIT_THE_NUMBER_OF_TCP_UDP_PORTS_USED_BY_INTERNAL_USERS"); %></p>
</div>
...
<table>
 <tr>
  ...
  <td>
   <input type="radio" value="0" name="connLimitcap" <% checkWrite("connLimit-cap0"); %> onClick="updateState()"><% multilang("247" "LANG_DISABLE"); %>&nbsp;&nbsp;
   <input type="radio" value="1" name="connLimitcap" <% checkWrite("connLimit-cap1"); %> onClick="updateState()"><% multilang("248" "LANG_ENABLE"); %>&nbsp;&nbsp;
  </td>
 </tr>
 <tr>
  <th><% multilang("632" "LANG_GLOBAL"); %> TCP <% multilang("23" "LANG_CONNECTION_LIMIT"); %>:</th>
  <td>
   <input type="text" name="tcpconnlimit" size="4" maxlength="4" value=<% getInfo("connLimit-tcp"); %>> &nbsp;(<% multilang("633" "LANG_0_FOR_NO_LIMIT"); %>) &nbsp;
  </td>
 </tr>
 <tr>
  <th><% multilang("632" "LANG_GLOBAL"); %> UDP <% multilang("23" "LANG_CONNECTION_LIMIT"); %>:</th>
  <td>
   <input type="text" name="udpconnlimit" size="4" maxlength="4" value=<% getInfo("connLimit-udp"); %> > &nbsp;(<% multilang("633" "LANG_0_FOR_NO_LIMIT"); %>) &nbsp;
  </td>
 </tr>
</table>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  ...
  <tr>
   <th><% multilang("634" "LANG_MAX_LIMITATION_PORTS"); %>:</th>
   <td>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("221" "LANG_ADD"); %>" name="addconnlimit" onClick="return addClick()">
...
<div class="column">
 <div class="column_title">
  ...
  <p><% multilang("635" "LANG_CURRENT_CONNECTION_LIMIT_TABLE"); %></p>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("224" "LANG_DELETE_SELECTED"); %>" name="deleteSelconnLimit" onClick="return deleteClick()">&nbsp;&nbsp;
 <input class="link_bg" type="submit" value="<% multilang("225" "LANG_DELETE_ALL"); %>" name="deleteAllconnLimit" onClick="return deleteAllClick()">&nbsp;&nbsp;&nbsp;
 <input type="hidden" value="/connlimit.asp" name="submit-url">
 <input type="hidden" name="postSecurityFlag" value="">
</div>
 <script>
